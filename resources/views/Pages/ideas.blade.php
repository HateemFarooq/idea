...
<div id="ideaModal"
    ...
    <div class="w-full max-w-lg flex flex-col max-h-[90vh] rounded-[2rem] border border-white/10 bg-[#0A0A0A] shadow-2xl relative">
        
        <div class="p-8 pb-4">
            <button id="closeIdeaModal" aria-label="Close modal" class="absolute top-6 right-6 text-gray-500 hover:text-white transition-colors">✕</button>
        ...
        <div class="flex-1 overflow-y-auto px-8 pb-8 
            ...
            <form id="ideaForm" method="POST" action="{{ route('ideas.store') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="status" class="w-full rounded-xl bg-white/[0.03] border border-white/10 px-4 py-3 text-white focus:outline-none focus:border-white/30 appearance-none [&>option]:bg-[#0A0A0A]">
    ...
    <div id="imagePreviewWrapper" class="mt-4 hidden">
        <img id="imagePreview" class="w-full h-48 object-cover rounded-xl border border-white/10">
                ...
                <!-- Steps -->
<div>
    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Steps</label>
    ...
    <button type="button" id="addStepBtn" class="mt-4 text-xs font-bold text-gray-500 hover:text-white transition-colors underline underline-offset-4">+ Add another step</button>
</div>

                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Links</label>
                    <div id="linksWrapper" class="space-y-3">
                        <div class="flex gap-2 link-row">
                            <input type="url" name="links[]" placeholder="https://example.com" class="flex-1 rounded-xl bg-white/[0.03] border border-white/10 px-4 py-3 text-white focus:outline-none focus:border-white/30">
                            <button type="button" class="remove-link px-3 rounded-xl border border-red-500/20 text-red-400/50 hover:text-red-400 hover:bg-red-500/10 transition-all">✕</button>
                        </div>
                    </div>
                    <button type="button" id="addLinkBtn" class="mt-4 text-xs font-bold text-gray-500 hover:text-white transition-colors underline underline-offset-4">+ Add another link</button>
                </div>
            </form>
        </div>

        <div class="p-8 pt-4 border-t border-white/5 flex justify-end gap-3">
            <button type="button" id="cancelIdeaModal" class="px-5 py-2 rounded-xl text-gray-400 hover:text-white transition-colors">Cancel</button>
            <button type="submit" form="ideaForm" class="px-6 py-2 rounded-xl bg-white text-black font-bold hover:bg-gray-200 active:scale-95 transition-all">
                Create Idea
            </button>
        </div>
    </div>
